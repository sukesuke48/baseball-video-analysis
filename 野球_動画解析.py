...
def detect_best_bat_line(frame, wrist_px, elbow_px):
...
def build_bat_two_points(grip_end_px, bat_tip_px):
    if grip_end_px is None or bat_tip_px is None:
        return None

    bat_angle_deg = normalize_angle_deg(calculate_line_angle(grip_end_px, bat_tip_px))
    bat_length_px = distance(grip_end_px, bat_tip_px)

    result = {
        "bat_grip_end_x": float(grip_end_px[0]),
        "bat_grip_end_y": float(grip_end_px[1]),
        "bat_tip_x": float(bat_tip_px[0]),
        "bat_tip_y": float(bat_tip_px[1]),
...
def detect_baseball_bat_points(frame, left_wrist_px, right_wrist_px, left_elbow_px, right_elbow_px):
    h, w = frame.shape[:2]
    ...
    best_line = detect_best_bat_line(frame, wrist_px, elbow_px)
    ...
    if d1 <= d2:
        ...
        bat_tip_px = point_to_tuple_float(p2)
    else:
        grip_end_px = point_to_tuple_float(p2)
        bat_tip_px = point_to_tuple_float(p1)

    bat_result = build_bat_two_points(grip_end_px, bat_tip_px)
    if bat_result is None:
        return None

    bat_result["active_side"] = active_side
    bat_result["bat_detected"] = 1
    return bat_result


def draw_baseball_bat_points(frame, row):
    point_specs = [
        ("bat_grip_end", "GRIP", (0, 255, 0)),
        ("bat_tip", "TIP", (0, 0, 255)),
    ]
    ...
    if "bat_grip_end" in point_map and "bat_tip" in point_map:
        cv2.line(frame, point_map["bat_grip_end"], point_map["bat_tip"], (255, 255, 0), 2)

    if "bat_grip_end" in point_map and row.get("bat_angle_deg") is not None:
        start_pt = point_map["bat_grip_end"]
        angle_deg = float(row["bat_angle_deg"])
        length = 90
        rad = np.radians(angle_deg)
        end_x = int(start_pt[0] + length * np.cos(rad))
        end_y = int(start_pt[1] + length * np.sin(rad))
        cv2.arrowedLine(frame, start_pt, (end_x, end_y), (255, 255, 255), 2, tipLength=0.2)
...
def add_empty_bat_fields(row):
    keys = [
        "active_side",
        "bat_grip_end_x", "bat_grip_end_y",
        "bat_tip_x", "bat_tip_y",
        "bat_angle_deg",
        "bat_length_px",
        "bat_detected",
...
def add_bat_fields(row, bat_result):
    if bat_result is None:
        add_empty_bat_fields(row)
        return

    for key, value in bat_result.items():
        row[key] = value
    row["bat_detected"] = 1
    ...
    try:
        ...
        with mp_pose.Pose(
            ...
            while True:
                ...
                bat_result = detect_baseball_bat_points(
                    frame,
                    left_wrist_px,
                    right_wrist_px,
                    left_elbow_px,
                ...
                add_bat_fields(row, bat_result)

                draw_baseball_bat_points(annotated_frame, row)
                draw_text_unicode(
                    annotated_frame,
                    [
                        f"frame: {frame_index}",
...
def main():
    st.set_page_config(page_title="野球動画解析", layout="wide")
    st.title("野球動画解析 Web版")
    st.write("MP4動画をアップロードすると、骨格・バット推定付き動画、CSV、グラフ、静止画を作成します。")

    uploaded_file = st.file_uploader("野球動画を選択してください", type=["mp4"])
